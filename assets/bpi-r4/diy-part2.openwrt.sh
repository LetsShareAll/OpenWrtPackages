...
clone_repo() {
  ...
  if [[ -d "${target}" ]]; then
    ...
    for attempt in {1..3}; do
      ...
      if git clone -b "${branch}" ${args} "${repo}" "${target}"; then
        break
      else
...
OPENCLASH_DIR="feeds/luci/applications/luci-app-openclash"
# if [[ -d "${OPENCLASH_DIR}" ]]; then
#   rm -rf "${OPENCLASH_DIR}"
# fi
clone_repo https://github.com/vernesong/OpenClash dev "--depth=1" \
  "${OPENCLASH_DIR}"
ARGON_THEME_DIR="feeds/luci/themes/luci-theme-argon"
# if [[ -d "${ARGON_THEME_DIR}" ]]; then
#   rm -rf "${ARGON_THEME_DIR}"
# fi
clone_repo https://github.com/jerrykuku/luci-theme-argon.git master "--depth=1" \
  "${ARGON_THEME_DIR}"

# Clone custom packages
clone_repo https://github.com/zhanghua000/luci-app-nginx master "--depth=1" \
  package/luci-app-nginx
clone_repo https://github.com/sundaqiang/openwrt-packages-backup main "--depth=1" \
  package/sundaqiang
clone_repo https://github.com/rockjake/luci-app-fancontrol.git main "--depth=1" \
  package/fancontrol
clone_repo https://github.com/gdy666/luci-app-lucky.git main "--depth=1" \
  package/lucky
clone_repo https://github.com/anoixa/bpi-r4-pwm-fan main "--depth=1" \
  package/bpi-r4-pwm-fan
clone_repo https://github.com/sbwml/openwrt-qBittorrent master "--depth=1" \
  package/qbittorrent
# clone_repo https://github.com/QiuSimons/luci-app-daed kix "--depth=1" \
#   package/daed
clone_repo https://github.com/jerrykuku/luci-app-argon-config master "--depth=1" \
  package/argon-config
clone_repo https://github.com/sirpdboy/luci-app-ddns-go main "--depth=1" \
  package/ddns-go
clone_repo https://github.com/zhengmz/luci-app-zerotier master "--depth=1" \
  package/zerotier
clone_repo https://github.com/immortalwrt/luci master "--filter=blob:none --sparse --depth=1" \
  package/luci
cd package/luci && git sparse-checkout set applications/luci-app-dae applications/luci-app-daed && cd ../../
clone_repo https://github.com/immortalwrt/packages master "--filter=blob:none --sparse --depth=1" \
  package/packages
cd package/packages && git sparse-checkout set lang/golang net/dae net/daed && cd ../../
